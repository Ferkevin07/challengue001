...
//    String obtenerReporte();
    ...
    @Query("SELECT new com.ntt_data.service003.model.Reporte(m.fecha,c.numero_cuenta,c.tipo,c.saldo_inicial,c.estado,m.valor,m.saldo) FROM Movimiento m, Cuenta c WHERE c.cuenta_id = m.movimiento_id AND m.movimiento_id = 3 AND CAST(m.fecha AS date) BETWEEN :inicio AND :fin")
